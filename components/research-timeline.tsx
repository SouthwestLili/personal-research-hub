        ...
        <section key={year} aria-labelledby={`year-${year}`} className="grid gap-7 md:grid-cols-[7rem_1fr]">
          ...
          <ol className="relative border-l border-border pl-7 sm:pl-10">
            {items.map((activity) => {
              ...
              const isClickable = activity.visibility === 'PUBLIC' && Boolean(activity.href);
              ...
              const details = (
                <div
                  ...
                  className={`paper-panel rounded-xl p-5 sm:p-6 ${
                    isPrivate
                      ? 'cursor-default border-border/50 bg-muted/35 opacity-70'
                      : isClickable
                        ? 'cursor-pointer transition-all group-hover:border-primary/50 group-hover:bg-card group-hover:shadow-sm'
                        : 'cursor-default'
                  }`}
                >
                  <div className="flex flex-wrap items-center gap-3 text-xs font-semibold tracking-[0.08em] text-muted-foreground uppercase">
                    <span className={`inline-flex items-center gap-2 ${isPrivate ? 'text-muted-foreground' : 'text-primary'}`}><Icon className="size-4" aria-hidden="true" />{meta.label}</span>
                    {isPrivate && <span className="text-muted-foreground/70">Private content</span>}
                  </div>
                  <h3 className={`mt-3 font-heading text-2xl tracking-[-0.02em] ${isPrivate ? 'text-muted-foreground' : 'text-foreground'}`}>{activity.publicTitle}</h3>
              ...
              return (
                <li key={activity.id} className="relative grid gap-4 pb-7 last:pb-0 sm:grid-cols-[4.5rem_1fr]">
                  ...
                  <time dateTime={activity.date} className={`pt-3 text-sm text-muted-foreground ${isPrivate ? 'opacity-60' : ''}`}>
                  ...
                  {isClickable && activity.href ? (
                    <Link href={activity.href} className="group cursor-pointer rounded-xl focus-visible:outline-2 focus-visible:outline-offset-3">{details}</Link>
                  ) : details}
                </li>
              );
            })}
